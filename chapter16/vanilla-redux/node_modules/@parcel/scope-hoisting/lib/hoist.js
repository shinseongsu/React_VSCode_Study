...
const VISITOR = {
  ...
  AssignmentExpression(path, asset) {
    ...
    if (t.matchesPattern(left, 'module.exports') && !path.scope.hasBinding('module')) {
      let exportsId = getExportsIdentifier(asset, path.scope);
      asset.meta.isCommonJS = true;
      asset.symbols.set('*', exportsId.name);

      if (path.scope === path.scope.getProgramParent() && !path.scope.getBinding(exportsId.name) && path.parentPath.isStatement()) {
        let [decl] = path.parentPath.replaceWith(t.variableDeclaration('var', [t.variableDeclarator(exportsId, right)]));
