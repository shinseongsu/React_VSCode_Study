...
exports.validateExtends = validateExtends;
exports.validateExtendsConfig = validateExtendsConfig;
...
function validateConfigFile(config, relativePath) {
  ...
  validateMap(config.transforms, validatePipeline.bind(this), 'transformer', 'transforms', relativePath);
  validateMap(config.validators, validatePipeline.bind(this), 'validator', 'validators', relativePath);
  validatePackageName(config.bundler, 'bundler', 'bundler', relativePath);
  validatePipeline(config.namers, 'namer', 'namers', relativePath);
  validateMap(config.runtimes, validatePipeline.bind(this), 'runtime', 'runtimes', relativePath);
  validateMap(config.packagers, validatePackageName.bind(this), 'packager', 'packagers', relativePath);
  validateMap(config.optimizers, validatePipeline.bind(this), 'optimizer', 'optimizers', relativePath);
...
function validateExtends(exts, relativePath) {
...
function validateExtendsConfig(ext, relativePath) {
  if (!ext.startsWith('.')) {
    validatePackageName(ext, 'config', 'extends', relativePath);
  }
}

function validatePipeline(pipeline, pluginType, key, relativePath) {
  if (!pipeline) {
    return;
  }
...
function validateMap(globMap, validator, pluginType, configKey, relativePath) {
  if (!globMap) {
    return;
  }
  ...
  for (let k in globMap) {
    ...
    validator(globMap[k], pluginType, `${configKey}["${k}"]`, relativePath);
  }
} // Reasoning behind this validation:
// https://github.com/parcel-bundler/parcel/issues/3397#issuecomment-521353931


function validatePackageName(pkg, pluginType, key, relativePath) {
  if (!pkg) {
    return;
  }
