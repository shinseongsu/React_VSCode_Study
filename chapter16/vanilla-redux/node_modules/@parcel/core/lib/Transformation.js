...
class Transformation {
  ...
  async runPipeline(pipeline, initialAsset) {
    ...
    let processedFinalAssets = (_ref = (_processedCacheEntry = processedCacheEntry) !== null && _processedCacheEntry !== void 0 ? _processedCacheEntry : await pipeline.postProcess(finalAssets)) !== null && _ref !== void 0 ? _ref : [];
...
class Pipeline {
    ...
    for (let transformer of this.transformers) {
      ...
      for (let asset of inputAssets) {
        ...
        if (asset.value.type !== initialType) {
          ...
          try {
            let transformerResults = await this.runTransformer(asset, transformer.plugin, transformer.name, transformer.config);
  ...
  async runTransformer(asset, transformer, transformerName, preloadedConfig) {
